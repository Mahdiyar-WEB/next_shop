...
import { Coupon, Product } from "lib/models";
import { ApiError, body, currentUser, fail, objectId, ok } from "lib/api";
async function cart(user: Awaited<ReturnType<typeof currentUser>>) {
  await user.populate([
    {
      path: "cart.products.productId",
      select: "title slug price offPrice imageLink countInStock",
    },
    { path: "cart.coupon" },
  ]);
...
export async function GET(request: Request) {
  try {
    ...
    return ok({ cart: await cart(user) });
  } catch (error) {
    return fail(error);
  }
}
...
export async function DELETE(request: Request) {
  try {
    const user = await currentUser(request as never);
    const { productId, removeCoupon } = await body<{
      productId?: string;
    ...
    if (removeCoupon) user.set("cart.coupon", null);
    else {
      if (!productId) throw new ApiError(400, "شناسه محصول الزامی است");
      const item = userCart.products.find(
        (value) => value.productId.toString() === objectId(productId),
      );
      if (!item) throw new ApiError(404, "محصول در سبد نیست");
