...
export async function POST(request: Request) {
  try {
    const { phoneNumber } = await body<{ phoneNumber?: string }>(
      request as never,
    );
    if (!phoneNumber || !phonePattern.test(phoneNumber.trim()))
      throw new ApiError(400, "شمارهٔ موبایل معتبر نیست");
    ...
    const payload = new URLSearchParams({
      receptor: phoneNumber.trim(),
      token: code,
      template: process.env.KAVENEGAR_OTP_TEMPLATE || "registerVerify",
    });
